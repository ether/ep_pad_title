# Contributing to ep_pad_title

Thanks for helping improve `ep_pad_title` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_pad_title` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_pad_title from your local clone
pnpm run plugins i --path ../ep_pad_title

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_pad_title/` are picked up after a server restart.
...
* Etherpad version, ep_pad_title version, browser + OS
* Steps to reproduce
